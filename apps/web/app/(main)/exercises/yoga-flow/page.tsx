...
// Track hip hinge (forward fold): L_SHOULDER(11)->L_HIP(23)->L_KNEE(25)
// Folded: ~60° (range 50–80), Standing: ~170° (range 160–180)
const L_SHOULDER = 11, L_HIP = 23, L_KNEE = 25
const R_SHOULDER = 12, R_HIP = 24, R_KNEE = 26

const STANDING_THRESHOLD = 155   // standing ≥ 155°
const FOLD_THRESHOLD     = 90    // folded ≤ 90°
const GOOD_FOLD_MIN      = 50
const GOOD_FOLD_MAX      = 80
const HOLD_REP_FRAMES    = 30    // hold 30 frames of good form = 1 rep
...
export default function YogaFlowPage() {
  ...
  const stateRef      = useRef({ stage: 'standing', smoothed: 0, holdFrames: 0, reps: 0, started: false })
  ...
  useEffect(() => {
    ...
    if (frameBufferRef.current.length === 10) {
      ...
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ frames: frameBufferRef.current, age: 25, weight: 70.0, exercise_type: 'yoga' }))
      }
  ...
  const detect = useCallback(() => {
    ...
    if (results.landmarks.length > 0) {
      ...
      const leftOk  = Math.min(vis(L_SHOULDER), vis(L_HIP), vis(L_KNEE)) >= MIN_VIS
      const rightOk = Math.min(vis(R_SHOULDER), vis(R_HIP), vis(R_KNEE)) >= MIN_VIS

      let angle = 0
      if (leftOk && rightOk)     angle = (calcAngle(pt(L_SHOULDER), pt(L_HIP), pt(L_KNEE)) + calcAngle(pt(R_SHOULDER), pt(R_HIP), pt(R_KNEE))) / 2
      else if (leftOk)           angle = calcAngle(pt(L_SHOULDER), pt(L_HIP), pt(L_KNEE))
      else if (rightOk)          angle = calcAngle(pt(R_SHOULDER), pt(R_HIP), pt(R_KNEE))

      if (angle > 0) s.smoothed = s.smoothed ? ANGLE_SMOOTH * angle + (1 - ANGLE_SMOOTH) * s.smoothed : angle
      const sa = s.smoothed

      if (sa >= STANDING_THRESHOLD) {
        s.stage = 'standing'
        s.holdFrames = 0
      } else if (sa <= FOLD_THRESHOLD) {
        s.started = true
        s.stage = 'fold'
      }
      ...
      if (s.started) {
        if (s.stage === 'fold') {
          if (sa >= GOOD_FOLD_MIN && sa <= GOOD_FOLD_MAX) {
            feedback = '✓ Good fold — hold it!'
            s.holdFrames++
            if (s.holdFrames > 0 && s.holdFrames % HOLD_REP_FRAMES === 0) s.reps++
          } else if (sa > GOOD_FOLD_MAX) {
            feedback = 'Fold deeper — reach toward the floor'
            s.holdFrames = 0
          } else {
            feedback = '✓ Deep fold — breathe and hold'
            s.holdFrames++
            if (s.holdFrames > 0 && s.holdFrames % HOLD_REP_FRAMES === 0) s.reps++
          }
        } else if (s.stage === 'standing') {
          feedback = '✓ Standing tall — fold forward'
        }
      }
  ...
  const start = useCallback(async () => {
    setErr('')
    await startCamera()
    stateRef.current = { stage: 'standing', smoothed: 0, holdFrames: 0, reps: 0, started: false }
  ...
  const instructions = [
    ...
    "Exhale and hinge at the hips — fold forward",
    "Aim for hip angle ~60° (hands toward floor)",
    "Hold the fold for a breath, then rise back up",
    "Each 1-second hold in good form counts as a rep",
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="relative">
        ...
        <h1 className="text-4xl font-bold text-white mb-2">Yoga Flow</h1>
        <p className="text-neutral-400">Flexibility and mindful movement. AI tracks hip hinge depth and hold time — runs in your browser.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="relative aspect-video bg-neutral-900 rounded-lg overflow-hidden">
            ...
            {running && (
              <div className="absolute top-4 left-4 bg-black/70 backdrop-blur-sm rounded-lg p-3 space-y-1 z-30">
                <div className="text-white"><span className="text-neutral-400 text-sm">Hold Reps:</span><span className="text-2xl font-bold ml-2">{stats.reps}</span></div>
        ...
        <Card className="bg-black/[0.96] border-white/10 p-6">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-2 rounded-lg bg-green-500/20"><Activity className="h-5 w-5 text-green-400" /></div>
          ...
          <div className="mt-6 pt-6 border-t border-white/10">
            ...
            <ul className="space-y-2 text-sm text-neutral-400">
              ...
              <li>• Move slowly — hold each pose</li>
              <li>• Target hip angle: ~60° at fold</li>

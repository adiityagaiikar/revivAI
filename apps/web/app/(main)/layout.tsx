...
const clinicalMenuItems = [
  { name: 'Care Plan',          href: '/care-plan',          icon: Calendar },
  { name: 'Clinic Connect',     href: '/clinic-connect',     icon: MessageSquare },
  ...
  return (
    ...
    >
      ...
      <div className="p-6 border-b border-white/8">
        <Link href="/dashboard" className="text-xl font-bold text-white tracking-tight hover:opacity-80 transition-opacity">
      ...
      <nav className="flex-1 p-4 overflow-y-auto space-y-4">
            ...
            return (
              <Link key={item.name} href={item.href}
                className={cn(
                  ...
                  isActive ? 'bg-white/10 text-white border border-white/15' : 'text-white/40 hover:text-white hover:bg-white/5'
                )}
              >
                <Icon className={cn('h-4 w-4 shrink-0 transition-colors', isActive ? 'text-violet-400' : 'text-white/40 group-hover:text-white/70')} />
        ...
        <div>
          <p className="px-4 pb-2 text-[10px] font-semibold text-white/20 uppercase tracking-widest">Clinical</p>
          <div className="space-y-1">
        ...
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/8">
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <>
                <p className="text-sm font-medium text-white truncate leading-tight">{user?.name ?? 'Unknown'}</p>
                <p className="text-xs text-white/35 truncate leading-tight">
                  {user?.email ?? '—'}
                  {user?.role && (
                    <span className="ml-1.5 text-[10px] font-semibold text-violet-400/70 uppercase">· Patient</span>
                  )}
                </p>
              </>
            )}
